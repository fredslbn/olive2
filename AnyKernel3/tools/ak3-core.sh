...
split_boot() {
  ...
  if [ -f "$bin/unpackelf" ] && $bin/unpackelf -i $bootimg -h -q 2>/dev/null; then
    ...
    if [ -f boot.img-dt -a ! -f "$bin/elftool" ]; then
      case $(od -ta -An -N4 boot.img-dt | $gnused -e 's/ del//' -e 's/   //g') in
...
unpack_ramdisk() {
  ...
  if [ -f ramdisk.cpio ]; then
    comp=$($bin/magiskboot decompress ramdisk.cpio 2>&1 | grep -v 'raw' | $gnused -n 's;.*\[\(.*\)\];\1;p');
  else
...
repack_ramdisk() {
  ...
  if [ -f "$bin/mkmtkhdr" -a -f "$split_img/boot.img-base" ]; then
    mtktype=$(od -ta -An -N8 -j8 $split_img/ramdisk.cpio.gz-mtk | $gnused -e 's/ nul//g' -e 's/   //g' | tr '[:upper:]' '[:lower:]');
    case $mtktype in
    ...
    case $kernel in
      *Image*)
        ...
        if [ $((magisk_patched & 3)) -eq 1 ]; then
          ui_print " " "Magisk detected! Patching kernel so";
          ui_print "reflashing Magisk is not necessary...";
          comp=$($bin/magiskboot decompress kernel 2>&1 | grep -vE 'raw|zimage' | $gnused -n 's;.*\[\(.*\)\];\1;p');
...
patch_fstab() {
  ...
  if [ ! "$(echo "$entry" | grep "$6")" -o "$6" == " " -o ! "$6" ]; then
    ...
    newpart=$(echo "$part" | $gnused -e "s;${5};${6};" -e "s; ;;g" -e 's;,\{2,\};,;g' -e 's;,*$;;g' -e 's;^,;;g');
    newentry=$(echo "$entry" | $gnused "s;${part};${newpart};");
    $gnused -i "s;${entry};${newentry};" $1;
  fi;
}
...
patch_cmdline() {
  ...
  if ! grep -q "$1" $cmdfile; then
    ...
    $gnused -i -e 's;^[ \t]*;;' -e 's;  *; ;g' -e 's;[ \t]*$;;' $cmdfile;
  else
    match=$(grep -o "$1.*$" $cmdfile | cut -d\  -f1);
    $gnused -i -e "s;${match};${2};" -e 's;^[ \t]*;;' -e 's;  *; ;g' -e 's;[ \t]*$;;' $cmdfile;
  fi;
  if [ -f "$home/cmdtmp" ]; then
    $gnused -i "s|^cmdline=.*|cmdline=$(cat $cmdfile)|" $split_img/header;
    rm -f $cmdfile;
  fi;
}
...
setup_ak() {
  ...
  name=$(basename $block | $gnused -e 's/_a$//' -e 's/_b$//');
